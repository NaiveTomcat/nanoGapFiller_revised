...
def extract_node_attrs(node_declaration: str) -> dict:
...
def check_all_attrs_present(
    attr_mapping: dict, attrs: Collection=("name", "length", "cov", "seq")
) -> None:
...
def add_node_to_digraph(digraph: nx.DiGraph, node_attrs: dict) -> None:
...
def parse_fastg(f: str) -> nx.DiGraph:
    ...
    with open(f, "r") as graph_file:
        for line in graph_file:
            ...
            if stripped_line.startswith(">"):
                ...
                seq = Seq(stripped_line)
                if "seq" in curr_node_attrs:
                    curr_node_attrs["seq"] += seq
                else:
                    curr_node_attrs["seq"] = seq
    ...
    for node_name in digraph.nodes:
        check_all_attrs_present(
            digraph.nodes[node_name], attrs=("length", "cov", "seq")
        )
